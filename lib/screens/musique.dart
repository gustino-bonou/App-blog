...
class _MusiqueArticle extends State<MusiqueArticle> {
  ...
  Future<Article> getLateArticlesFromCollection() async {
    ...
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('articles').where("category", isEqualTo: "Music").where("isApproved", isEqualTo: true).orderBy("datePosted", descending: true).get();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        ...
        drawer: MyDrawer(),
        backgroundColor:  Color(0xFF4391FF),
            ...
            children: [
              ...
              StreamBuilder<QuerySnapshot>(
                stream: articlesRef.orderBy('datePosted', descending: true)
                    .where('category', isEqualTo: "Music").where("isApproved", isEqualTo: true).snapshots(),
